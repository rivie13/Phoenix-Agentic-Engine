...
# Create task issue — Phoenix Agentic Engine
...
- Check for existing issues covering the same scope before creating a new one:
  ```
  mcp_github_github_search_issues(owner="rivie13", repo="Phoenix-Agentic-Engine", query="<keywords>")
  ```
...
|-------|----------------|----------|---------|
| Target repo | `### Target repository` | Yes | `Phoenix-Agentic-Engine` |
| Parent branch | `### Parent branch (PR target)` | Yes | `feature/phase-3-mcp-client` |
| Working branch | `### Working branch` | Yes | `subfeature/task/mcp-client-core` |
...
| Lock key | `### Lock key` | No | `gateway/auth/*` |
...
subfeature/task/mcp-client-core
...
Implement the core MCP client connection manager...
